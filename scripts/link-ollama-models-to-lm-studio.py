...
def iter_leaf_directories(root_path):
    root = Path(root_path)
    
    # rglob("*") recursively finds all files and folders
    # We filter for directories that have no directory-type children
    return (
...
def main():
    ollama_library_path = iter_leaf_directories(OLLAMA_MODEL_DIR / "manifests")
    ...
    for model_family_path in ollama_library_path:
